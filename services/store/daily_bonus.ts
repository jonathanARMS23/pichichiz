...
export default class DailyBonusStore extends API {
    private URL = `${SOCKET_SERVER_URL}/api`

    public Init = async (id_user: number) => {
        try {
            ...
            const response = (
                await axios.get(`${this.URL}/dailybonus/init/${id_user}`, {
                    headers: {
    ...
    public GetDailyBonus = async (id_user: number) => {
        try {
            const dataToken = await AsyncStorage.getItem('socket_token')
            if (!dataToken) return { canceled: true }
            ...
            const response = (
                await axios.get(`${this.URL}/dailybonus/${id_user}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                })
    ...
    public WinBonus = async (id_user: number) => {
        try {
            const dataToken = await AsyncStorage.getItem('socket_token')
            if (!dataToken) return { canceled: true }
            ...
            const response = (
                await axios.get(`${this.URL}/dailybonus/checked/${id_user}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                })
