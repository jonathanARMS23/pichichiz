...
import DynamicRate from '../duelgame/Finished/DynamicRate'
...
export default ({ serie, isFinished, Duel }: IProps) => {
        ...
        ;(async () => {
            if (isSubscribed) {
                ...
                if (!response.canceled) {
                    /** if (player === 1) {
                        if (response.score1) {
                            setScore(parseInt(`${response.score1}`, 10))
                            setResult_player1(response.player1Road)
                        } else {
                            setScore(0)
                            setResult_player1(null)
                        }
                    } else {
                        if (response.score2) {
                            setScore(parseInt(`${response.score2}`, 10))
                            setResult_player1(response.player1Road)
                            setResult_player2(response.player2Road)
                        } else {
                            setScore(0)
                            setResult_player2(null)
                        }
                    } */
                    setScore(
                        player === 1
        ...
        <View style={{ ...Style.wrapper, minWidth: width, maxWidth: width }}>
            ...
            <View style={Style.Series}>
                ...
                <View style={Style.seriesList}>
                    ...
                    {/** <Serie serie={serie} series={1} />
                    <Serie serie={serie} series={2} />
                    <Serie serie={serie} series={3} />
                    <Serie serie={serie} series={4} />
                    <Serie serie={serie} series={5} /> */}
                </View>
            </View>
            ...
            <Text style={{ color: '#FFFFFF', marginHorizontal: 10 }}>
                {isFinished
                    ...
                    : parseInt(`${User.id}`, 10) ===
                      parseInt(`${Duel.id_player1}`, 10)
                    ? `Pour continuer les autres séries, tu dois attendre que ${
                          Duel.pseudo_player2 ?? 'le joueur random'
                      } ait fini cette série.`
                    : `Pour continuer les autres séries, tu dois attendre que ${Duel.pseudo_player1} ait fini la prochaine série.`}
            </Text>
        </View>
    )
}
